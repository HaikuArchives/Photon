...
class TTestGIMPWindow : public BWindow {
public:
				TTestGIMPWindow (void);
				~TTestGIMPWindow ();
...
//	Window class.
//-------------------------------------------------------------------------------------------------
TTestGIMPWindow::TTestGIMPWindow (void) : BWindow (BRect (20, 50, 620, 460), "Test Filters", B_DOCUMENT_WINDOW, B_ASYNCHRONOUS_CONTROLS) {
	fView = NULL; fSrcBitmap = fRealSrc = fDstBitmap = fRealDst = NULL;
	gWindow = this;
	BRect r;
	r.Set(0, 0, 32767, 15); fMenuBar = new BMenuBar(r, "MenuBar");
	BuildMenus ();
	Lock();
	AddChild(fMenuBar);
	fMenuBarHeight = int32(fMenuBar->Bounds().bottom) + 1;
	Unlock();
	Show ();
}

TTestGIMPWindow::~TTestGIMPWindow () {
...
void GimpMenuInstaller (const char *menu, const char *item, BMessage *msg) {
	...
	menuMsg->what = 'Fltr';
	gWindow->AddFilter (menu, item, menuMsg);
}
...
void TTestGIMPWindow::BuildMenus (void) {
	BMenu		*menu;
	BMenuItem	*item;
	...
	RegisterGimpFilters ();
	fMenuBar->AddItem(fFilterMenu); fFilterMenu->SetEnabled (false);
}

void TTestGIMPWindow::AddFilter (const char *menu, const char *item, BMessage *msg) {
	fFilterMenu->AddItem (new BMenuItem (item, msg));
}

void TTestGIMPWindow::UseBitmap (BBitmap *bm) {
	Lock ();
	fFilterMenu->SetEnabled (true);
	if (fView) { RemoveChild (fView); delete fView; }
	ResizeTo (bm->Bounds().IntegerWidth (), bm->Bounds().IntegerHeight () + fMenuBarHeight);
	BRect	r = Bounds (); r.top += fMenuBarHeight;
...
void TTestGIMPWindow::SwitchBuffers (bool showMain) {
	bool	doUpdate;
	if (showMain) {
...
void BufferSwitcherProc (struct TFilterParams *parms, bool showMain) {
	gWindow->SwitchBuffers (showMain);
}

void TTestGIMPWindow::Update (void) {
...
int32 AsyncFilter (void *arg) {
	...
	if (unarchived) {
		...
		parms.fCurSel = NULL;
		FakeTBitmap		srcBits(gWindow->Src ()), dstBits(gWindow->Dst ());
...
void TTestGIMPWindow::MessageReceived (BMessage *msg) {
	switch (msg->what) {
